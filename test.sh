...
info "List files in /usr/fontfixing"
docker run --rm $PLATFORM -v $(pwd):/data -w /data --entrypoint ls $DOCKER_IMAGE /usr/fontfixing/
...
rm -f $TEMPORARY_DIRECTORY/7.21.7-t02-fail-a-fixed.pdf
